...
const generatedArticlePreviewCss = await readFile(path.join(publicRoot, '_generated', 'article-preview.css'), 'utf8')
const builtArticlePreviewCss = await readFile(path.join(distRoot, '_generated', 'article-preview.css'), 'utf8')
...
assert(builtArticlePreviewCss === generatedArticlePreviewCss, '部署产物中的文章预览样式与构建源不一致')
assert(generatedArticlePreviewCss.includes('.page-article-detail .vp-doc'), '文章预览样式缺少详情页排版规则')
