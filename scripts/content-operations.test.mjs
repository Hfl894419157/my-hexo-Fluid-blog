...
test('FAQ 首版包含三组各四题，并接入搜索锚点与 FAQPage 结构化数据', async () => {
  ...
  assert.doesNotMatch(faqPageSource, /PageHero/)
  assert.doesNotMatch(faqPageSource, /集中回答合作、资源获取和网站使用中的常见疑问/)
...
test('索引页只移除指定页面的顶部介绍与页内搜索', async () => {
  ...
  assert.match(blog, /:show-hero="false"/)
  assert.match(blog, /:section-description="description"/)
  assert.doesNotMatch(blog, /上方搜索栏/)
  assert.match(resources, /:show-hero="false"/)
  assert.doesNotMatch(resources, /resources-page__filter|BaseButton/)
